...
//etcSymbol => /module/lib.php에서 선언된 배열
foreach($etcSymbol as $s => $t){
	...
	foreach($row as $v){
		...
		//가장 최근값은 Stock_Candles_Last 테이블 데이터 이용(오전 6시반에 Quote로 업데이트됨)
		if(date('n/d',$v['t']) == date('n/d',$lastData['t'])){
			$v['c'] = $lastData['c'];
			$v['t'] = $lastData['t'];
		}
	...
	$v['pmDataDay'] = Util::fnPercent($row[$rowCnt-2]['c'],$v['c']);		//증감률
