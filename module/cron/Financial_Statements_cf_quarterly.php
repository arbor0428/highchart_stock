...
$logFile = '/home/myss/www/module/cron/log/Financial_Statements_cf_quarterly.log';
...
/*
		if($symbol != $reSymbol){
			sqlExe("update api_Financial_Statements_cf_quarterly set reSymbol='".$reSymbol."' where symbol='".$symbol."'");
		}
*/
		...
		foreach($result as $item){
			...
			if($capex){
				$tmpChk = sqlRowOne("select count(*) from api_Financial_Statements_cf_quarterly where symbol='".$symbol."' and period='".$period."'");
				if(!$tmpChk){
					$sql = "insert into api_Financial_Statements_cf_quarterly (symbol,capex,cashDividendsPaid,cashInterestPaid,cashTaxesPaid,changeinCash,changesinWorkingCapital,deferredTaxesInvestmentTaxCredit,depreciationAmortization,issuanceReductionCapitalStock,issuanceReductionDebtNet,netCashFinancingActivities,netIncomeStartingLine,netInvestingCashFlow,netOperatingCashFlow,otherFundsFinancingItems,otherFundsNonCashItems,otherInvestingCashFlowItemsTotal,period,reSymbol) values  ('$symbol','$capex','$cashDividendsPaid','$cashInterestPaid','$cashTaxesPaid','$changeinCash','$changesinWorkingCapital','$deferredTaxesInvestmentTaxCredit','$depreciationAmortization','$issuanceReductionCapitalStock','$issuanceReductionDebtNet','$netCashFinancingActivities','$netIncomeStartingLine','$netInvestingCashFlow','$netOperatingCashFlow','$otherFundsFinancingItems','$otherFundsNonCashItems','$otherInvestingCashFlowItemsTotal','$period','$reSymbol')";
					sqlExe($sql);
				}else{
					break;
				}
