...
function pageNumCount(fname,rs){
	rc = $('#'+fname+'_rc').val();
...
function DocFileSizeChk(fname,fsize,no){
	...
	if( upFile != "" ){
		...
		if(fS > fC){
			alert('파일은 '+fsize+'MB이상 등록할 수 없습니다.');
			$("#upfile"+no).val('');
			$("#file_route"+no).val('');
			return;
		}
...
function symbolData(s,u,t){
	if(t == 'earnings')			h = 350;
	else if(t == 'dividends')	h = 350;
	else if(t == 'ipo')			h = 350;
	else							h = 0;

	if(h){
		$("#multiBox").css({"width":"90%","max-width":"650px"});
		$('#multi_ttl').text(s);
		$('#multiFrame').html("<iframe src='/calendarData.php?symbol="+s+"&uid="+u+"&dataType="+t+"' name='calendarData' style='width:100%;height:"+h+"px;' frameborder='0' scrolling='auto'></iframe>");
		$('.multiBox_open').click();
	}
}