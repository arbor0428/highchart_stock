...
$logFile = '/home/myss/www/module/cron/log/Stock_Candles_W.log';
...
$resolution = 'W';		//ex)1, 5, 15, 30, 60, D, W, M
...
foreach($row as $k => $v){
	...
	//		echo '<br>';
		...
		foreach($resArr as $item){
			$tmpChk = sqlRowOne("select count(*) from api_Stock_Candles_W where symbol='".$symbol."' and t='".$item['t']."'");
			if(!$tmpChk){
				sqlExe("insert into api_Stock_Candles_W (symbol,c,h,l,o,v,t) values ('".$symbol."','".$item['c']."','".$item['h']."','".$item['l']."','".$item['o']."','".$item['v']."','".$item['t']."')");
			}
		}
