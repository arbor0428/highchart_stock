...
if($cancleBtnID){
	...
	$cancleBtnArr = explode(',',$cancleBtnID);
	for($i=0; $i<count($cancleBtnArr); $i++){
		if($cancleBtnObj)	$cancleBtnObj .= ',';
		$cancleBtnObj .= '#'.$cancleBtnArr[$i];
	}
}
?>
...
<style>
.fpicker, .monthpicker, #fpicker1, #fpicker2, #fpicker3, #fpicker4, #fpicker5, #fpicker6, #fpicker7{
	background:url('http://i-web.kr/module/common/images/inputArrow.gif') no-repeat;
...
.ui-datepicker{font-size:12px;width:240px;font-size:14px;}
.ui-datepicker select.ui-datepicker-month{width:37%;height:24px;font-size:14px;}
.ui-datepicker select.ui-datepicker-year{width:40%;height:24px;font-size:14px;}

.ui-datepicker td span, .ui-datepicker td a{font-size:14px;}

.ui-datepicker-week-end.date-saturday a.ui-state-default{color:blue}
.ui-datepicker-week-end.date-sunday a.ui-state-default{color:red}
.date-holiday a.ui-state-default{color:red}
</style>
...
$(function(){
	$( ".fpicker, #fpicker1,#fpicker2,#fpicker3,#fpicker4,#fpicker5,#fpicker6,#fpicker7" ).datepicker({
...
$(document).ready(function () {
	$('.fpicker, #fpicker1,#fpicker2,#fpicker3,#fpicker4,#fpicker5,#fpicker6,#fpicker7').prop('readonly', true);

	//취소버튼 활성화
	if('<?=$cancleBtnObj?>'){
		$(document).find("<?=$cancleBtnObj?>").removeClass('hasDatepicker').datepicker({
			dateFormat: 'yy-mm-dd',
			prevText: '이전 달',
			nextText: '다음 달',
			monthNames: ['1월','2월','3월','4월','5월','6월','7월','8월','9월','10월','11월','12월'],
